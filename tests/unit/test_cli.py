...
class TestSciPFSCLI(unittest.TestCase):
    ...
    def test_update_library_no_change(self, MockIPFSClient, MockLibrary):
        ...
        initial_lib_mock.manifest_path = MagicMock(spec=Path) # Explicitly make manifest_path a mock
        initial_lib_mock.manifest_path.exists.return_value = True
        initial_lib_mock.manifest_path.__str__.return_value = str(TEST_CONFIG_DIR / "updatelib_manifest.json")
        initial_lib_mock.manifest_path.name = (TEST_CONFIG_DIR / "updatelib_manifest.json").name

        initial_lib_mock.ipns_name = "/ipns/k51updatelib"
        initial_lib_mock.manifest_cid = "QmSameCID" # Ensure this is set as per test logic
    ...
    def test_update_library_with_changes(self, MockIPFSClient, MockLibrary):
        ...
        initial_lib_mock.manifest_cid = "QmOldCID" # Ensure this is set as per test logic
